  ...
  return (
    <Section id="experience" index="02" eyebrow="Experience" heading="The path so far.">
      <ol className="relative">
        ...
        <span
          aria-hidden="true"
          className="absolute left-[7px] top-2 bottom-2 w-px bg-gradient-to-b from-cyan/50 via-border to-transparent sm:left-[9px]"
        />
          ...
          <Reveal as="li" key={`${job.org}-${job.role}`} delay={(i % 4) * 0.05}>
            <div className="relative grid grid-cols-[auto_1fr] gap-x-5 pb-12 last:pb-0 sm:gap-x-7">
              ...
              <div className="relative mt-1.5 flex h-4 w-4 items-center justify-center sm:h-5 sm:w-5">
                <span className="absolute h-4 w-4 rounded-full bg-cyan/20 sm:h-5 sm:w-5" />
                <span
                  className={`h-2.5 w-2.5 rounded-full ${
                    job.current ? "bg-cyan shadow-[0_0_12px_2px_var(--color-cyan)]" : "bg-faint"
                  }`}
                />
              </div>
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm font-medium text-cyan/90">
                  {job.org}
